...
(() => {
  ...
  var SEAL_MODE = searchParams.has("seal");
  ...
  var MUSIC_SHOP_SOURCE_SCALE = 2;
  var MUSIC_SHOP_WIDTH = 543;
  var MUSIC_SHOP_HEIGHT = 724;
  ...
  var CINEMA_AUDIENCE_DIALOGUE_INDEX_KEY = "max-game:cinema-audience-dialogue-index";
  var CINEMA_AUDIENCE_LINES = [
    "Shhh!",
    "The film's on.",
  ...
  var CINEMA_AUDIENCE_INTERACTION_DISTANCE = 78;
  var CINEMA_AUDIENCE = [
    { x: 153, y: 373 },
    { x: 207, y: 375 },
    { x: 262, y: 373 },
  ...
  var SHOW_COLLISIONS = searchParams.has("collisions");
  var enteredDoor = searchParams.get("door");
  var isCinemaInterior = enteredDoor === "cinema";
  var isMusicShopInterior = enteredDoor === "music-shop";
  var isCaveInterior = enteredDoor === CAVE_DOOR_ID || enteredDoor === "cave";
  var isDiaryLabInterior = !isCinemaInterior && !isMusicShopInterior && !isCaveInterior;
  if (isMusicShopInterior) {
    document.title = "Music House";
    canvas.setAttribute("aria-label", "Music House interior");
  }
  var WORLD_WIDTH = isCaveInterior ? CAVE_WIDTH : isMusicShopInterior ? MUSIC_SHOP_WIDTH : DEFAULT_INTERIOR_WIDTH;
  var WORLD_HEIGHT = isCaveInterior ? CAVE_HEIGHT : isMusicShopInterior ? MUSIC_SHOP_HEIGHT : DEFAULT_INTERIOR_HEIGHT;
  var INTERACTION_TARGETS = [
    ...isCaveInterior ? [{ kind: "colander", label: "Pick up colander", x: CAVE_COLANDER.x, y: CAVE_COLANDER.y, distance: 78 }] : isDiaryLabInterior ? [
  ...
  interior.src = isCaveInterior ? "../img/internal/cave.jpg?v=20260825-colander-cave" : isCinemaInterior ? "../img/internal/cinema.png?v=20260831-six-seat-audience" : isMusicShopInterior ? "../img/internal/internal-music.png?v=20260831-interior" : "../img/internal/diary-lab.png";
  if (!isMusicShopInterior && !isCaveInterior) {
    collisionMask.src = isCinemaInterior ? "../img/internal/cinema-collisions.png?v=20260831-no-bottom-bench" : "../img/internal/diary-lab-collision.png";
    doorOverlay.src = isCinemaInterior ? "../img/internal/cinema-open-door.png" : "../img/internal/diary-lab-doors-out.png";
  }
  ...
  var collisionCellSize = isMusicShopInterior ? MUSIC_HOUSE_COLLISION_CELL_SIZE : isCinemaInterior ? CINEMA_COLLISION_CELL_SIZE : INTERNAL_COLLISION_CELL_SIZE;
  var collisionColumns = isMusicShopInterior ? MUSIC_HOUSE_COLLISION_COLUMNS : isCinemaInterior ? CINEMA_COLLISION_COLUMNS : INTERNAL_COLLISION_COLUMNS;
  var collisionRows = isMusicShopInterior ? MUSIC_HOUSE_COLLISION_ROWS : isCinemaInterior ? CINEMA_COLLISION_ROWS : INTERNAL_COLLISION_ROWS;
  var collisionBinary = atob(
    isMusicShopInterior ? MUSIC_HOUSE_COLLISION_BITS : isCinemaInterior ? CINEMA_COLLISION_BITS : INTERNAL_COLLISION_BITS
  );
  ...
  var player = {
    x: isCaveInterior ? 640 : isCinemaInterior ? 256 : isMusicShopInterior ? 272 : enteredDoor === "diary-lab-right" ? 359 : 153,
  ...
  var INTERIOR_DOORS = isCaveInterior ? CAVE_DOORS : isCinemaInterior ? CINEMA_DOORS : isMusicShopInterior ? MUSIC_SHOP_DOORS : DIARY_LAB_DOORS;
  var DOOR_OPEN_DISTANCE = 53;
  var DOOR_EXIT_DISTANCE = 18;
  var DOOR_PASSAGE_HALF_WIDTH = 24;
  var DOOR_PASSAGE_TOP_OFFSET = 30;
  var DOOR_PASSAGE_BOTTOM_OFFSET = 24;
  ...
  function loadCinemaAudienceDialogueIndex() {
    try {
      ...
      return Number.isFinite(storedIndex) && storedIndex >= 0 ? storedIndex % CINEMA_AUDIENCE_LINES.length : 0;
    } catch {
  ...
  function isCollisionPixel(x, y) {
    ...
    if (!isCaveInterior && openDoor && x >= openDoor.triggerX - DOOR_PASSAGE_HALF_WIDTH && x <= openDoor.triggerX + DOOR_PASSAGE_HALF_WIDTH && y >= openDoor.triggerY - DOOR_PASSAGE_TOP_OFFSET && y <= openDoor.exitY + DOOR_PASSAGE_BOTTOM_OFFSET) {
      return false;
    }
    if (isCaveInterior) {
  ...
  function updateNearbyInteraction() {
    ...
    if (isCinemaInterior) {
      nearbyInteraction = null;
      interactionPrompt.hidden = true;
      const nextAudienceIndex = CINEMA_AUDIENCE.map((audienceMember, index) => ({
        index,
        distance: Math.hypot(player.x - audienceMember.x, player.y - audienceMember.y)
      })).filter(({ distance }) => distance <= CINEMA_AUDIENCE_INTERACTION_DISTANCE).sort((first, second) => first.distance - second.distance)[0]?.index ?? null;
      ...
      if (previousAudienceIndex !== null && noelDialogueOpen && noelDialogueFollowsProximity) {
        closeNoelDialogue();
      }
    ...
    if (previousInteraction === "noel" && nextInteraction !== "noel" && noelDialogueOpen && noelDialogueFollowsProximity) {
      closeNoelDialogue();
    }
  ...
  function updateDoors() {
    ...
    const exitDoor = INTERIOR_DOORS.find(
      (door) => Math.hypot(player.x - door.exitX, player.y - door.exitY) <= DOOR_EXIT_DISTANCE
    );
    if (!exitDoor) return;
  ...
  syncInteriorExitLink();
  bindControls();
  var requiredImages = isCaveInterior ? [interior, spriteSheet] : isMusicShopInterior ? [interior, spriteSheet] : [interior, collisionMask, doorOverlay, spriteSheet, noelSprite];
