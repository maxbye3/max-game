...
import { MUSIC_HOUSE_COLLISION_BITS, MUSIC_HOUSE_COLLISION_CELL_SIZE, MUSIC_HOUSE_COLLISION_COLUMNS, MUSIC_HOUSE_COLLISION_ROWS, } from './music-house-collision-mask.js';
...
const noelDialogueNext = requireElement('#noel-dialogue-next');
...
const collisionBinary = atob(isMusicShopInterior
...
const SEAL_FRAME_X = [0, 130, 254, 380, 506, 630, 754, 881];
const SEAL_FRAME_WIDTH = [130, 124, 126, 126, 124, 124, 127, 126];
const SEAL_ROW_Y = [0, 162, 323, 486, 646, 805, 970, 1134, 1290];
const SEAL_ROW_HEIGHT = [162, 161, 163, 160, 159, 165, 164, 156, 164];
const player = {
    x: isCaveInterior
        ? 640
        : isCinemaInterior
...
let nearbyCinemaAudienceIndex = null;
...
function holdDirection(direction) {
    if (noelDialogueOpen && !noelDialogueFollowsProximity)
        return;
...
function loadCinemaAudienceDialogueIndex() {
    try {
        const storedIndex = Number.parseInt(window.localStorage.getItem(CINEMA_AUDIENCE_DIALOGUE_INDEX_KEY) ?? '0', 10);
...
function saveCinemaAudienceDialogueIndex(index) {
...
function startCinemaAudienceDialogue() {
    if (!isCinemaInterior || nearbyCinemaAudienceIndex === null || noelDialogueOpen)
        return;
...
function playerCollidesAt(x, y) {
    if (isDiaryLabInterior && Math.hypot(x - NOEL.x, y - NOEL.y) < NOEL_COLLISION_DISTANCE)
        return true;
...
function updateDoors() {
    ...
    if (navigationStarted)
        return;
    const exitDoor = INTERIOR_DOORS.find((door) => Math.hypot(player.x - door.exitX, player.y - door.exitY) <= DOOR_EXIT_DISTANCE);
    if (!exitDoor)
        return;
    navigationStarted = true;
    const returnParams = new URLSearchParams();
    if (enteredDoor)
        returnParams.set('door', enteredDoor);
    if (isCaveInterior && caveColanderHeld)
        returnParams.set('colander', '1');
    if (SEAL_MODE)
        returnParams.set('seal', '1');
...
function draw() {
    ...
    context.drawImage(interior, cameraX * interiorSourceScale, cameraY * interiorSourceScale, viewportWidth * interiorSourceScale, viewportHeight * interiorSourceScale, 0, 0, canvas.width, canvas.height);
    if (isCaveInterior && caveColanderHeld) {
        ...
        if (isCaveInterior) {
            ...
            for (let row = firstRow; row <= lastRow; row += 1) {
                for (let column = firstColumn; column <= lastColumn; column += 1) {
                    ...
                    if (!isCollisionPixel(x + collisionCellSize / 2, y + collisionCellSize / 2))
                        continue;
                    context.fillRect((x - cameraX) * VIEW_SCALE, (y - cameraY) * VIEW_SCALE, collisionCellSize * VIEW_SCALE, collisionCellSize * VIEW_SCALE);
                }
            }
        }
        else {
    ...
    context.drawImage(spriteSheet, sourceX, sourceY, sourceWidth, sourceHeight, Math.round((player.x - cameraX - width / 2) * VIEW_SCALE), Math.round((player.y - cameraY - height + baselineOffset) * VIEW_SCALE), width * VIEW_SCALE, height * VIEW_SCALE);
    if (caveColanderHeld) {
